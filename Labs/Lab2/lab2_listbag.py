...
class ListBag:
    ...
    def remove(self, item: any) -> any:
        ...
        """
        if self.__len__ == 0:
            raise ValueError("bag.remove(" + str(item) + "): remove from empty bag")
        if item not in self:
            raise ValueError("bag.remove(" + str(item) + "): item not in bag")
    ...
    def grab(self) -> any:
        ...
        if len(self._elems) == 0:
            raise ValueError("bag.remove(" + str(item) + "): remove from empty bag")
        self._elems.remove(self._elems[item])
    ...
    def __add__(self, other: 'ListBag') -> 'ListBag':
        ...
        return ListBag(other._elems + self._elems)
