...
class LinkedList:
    ...
    def __contains__(self, x: any) -> bool:
        ...
        for node in range(self._num_items):
            if cursor.item == x:
                return True
            cursor = cursor.next
    ...
    def __eq__(self, other: 'LinkedList') -> bool:
        ...
        False
        """
        if type(other) != LinkedList or self._num_items != other._num_items:
            return False
        
        if self._num_items == 0 and other._num_items == 0:
            return True
        
        self_cursor = self._first.next
        other_cursor = other._first.next
        while self_cursor is not None:
            if self_cursor.item != other_cursor.item:
                return False
    ...
    def remove(self, x: any) -> None:
        ...
        if self._first.next.item == x:
            self._first.next = self._first.next.next
        else:
            cursor = self._first.next
            while cursor.next.item != x:
                if cursor.next.next == None:
                    raise ValueError("LinkedList.remove(" + str(x) + "): " + str(x) + " not in list")
                cursor = cursor.next
            cursor.next = cursor.next.next     
        self._num_items -= 1       