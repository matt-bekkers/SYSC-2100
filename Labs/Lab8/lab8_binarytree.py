...
#         return 1 + max(self._height(node.left), self._height(node.right))
   ...
   # Exercise 7
    ...
    def count(self, item: any) -> int:
        ...
        def _traverse_count(node: 'BinaryTree._Node') -> int:
            if node is None:
                return 0
            if node.x == item:
                return 1 + _traverse_count(node.left) + _traverse_count(node.right)
            return _traverse_count(node.left) + _traverse_count(node.right)

        return _traverse_count(self._root)
...
if __name__ == "__main__":
    ...
    tree.preorder_print()
    print("preorder perfect")
    tree = build_perfect_binary_tree()
    tree.preorder_print()
    print("inorder bin")
    tree = build_binary_tree()
    tree.inorder_print()
    print("inorder perfect")
    tree = build_perfect_binary_tree()
    tree.inorder_print()
    print("postorder bin")
    tree = build_binary_tree()
    tree.postorder_print()
    print("postorder perfect")
    tree = build_perfect_binary_tree()
    tree.postorder_print()